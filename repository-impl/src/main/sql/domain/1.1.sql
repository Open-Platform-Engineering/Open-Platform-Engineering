...
create table cp_incidents (
    ...
    version integer not null,
    constraint ck_cp_incidents_incident_status check (
        incident_status in (0, 1, 2)
    ),
...
create table cp_incident_events (
    ...
    constraint ck_cp_incident_events_event_type check (
        event_type in (
            'INTEGRATION', 'WEB_APP', 'MOBILE_APP', 'API', 'EMAIL', 'SLACK'
        )
    ),
...
create table cp_services (
    ...
    constraint ck_cp_services_svc_status check (
        svc_status in (
            'ACTIVE', 'WARNING', 'CRITICAL', 'MAINTENANCE', 'DISABLED'
        )
    ),
    constraint ck_cp_services_urgency_strategy check (
        urgency_strategy in ('HIGH', 'LOW', 'DYNAMIC', 'BASED_ON_SUPPORT_HOURS')
    ),
